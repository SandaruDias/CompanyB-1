...
import { Box, Button, TextField, Typography, Alert, Snackbar,Toolbar,IconButton,AppBar,Drawer } from '@mui/material';
...
function FileUploadform() {
    ...
    const handleSubmit = (event) => {
        ...
        axios.post('http://localhost:8090/tps/simulations/new',simulationData)
        .then(response => {
            console.log('Server response:', response);
            setAlertInfo({ open: true, Message: "Simulation initiated successfully!", severity: "success" });
            navigate("/training-simulation");
        })
        .catch(error => {
            console.error('Failed to initiate simulation:', error);
            setAlertInfo({ open: true, Message: "Failed to initiate simulation. Please try again.", severity: "error" });
        });
    };
